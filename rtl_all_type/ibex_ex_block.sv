...
module ibex_ex_block #(
  ...
  input ibex_pkg::res_sel_e     result_sel_i,
  ...
  */
  ...
  always_comb begin : result_mux
    unique case (result_sel_i)
      FU_RES:          result_ex_o = fu_result;
      MULTDIV_RES:     result_ex_o = multdiv_result;
      ALU_RES:         result_ex_o = alu_result;
      default:         result_ex_o = alu_result;
    endcase
  end
  //assign result_ex_o  = multdiv_sel ? multdiv_result : alu_result;

  // branch handling
  always_comb begin : branch_decision_mux
    unique case (result_sel_i)
      FU_RES:          branch_decision_o = fu_result;
      default:         branch_decision_o = alu_cmp_result;
    endcase
  end
  ...
  if (BranchTargetALU) begin : g_branch_target_alu
    ...
    assign branch_target_o = (result_sel_i == FU_RES) ? fu_result : alu_adder_result_ex_o;
  end
  ...
  assign alu_valid = ~(|alu_imd_val_we)

  // Multiplier/divider may require multiple cycles. The ALU output is valid in the same cycle
  // unless the intermediate result register is being written (which indicates this isn't the
  // final cycle of ALU operation).
  always_comb begin : valid_result_mux
    unique case (result_sel_i)
      FU_RES:          ex_valid_o = fu_valid;
      MULTDIV_RES:     ex_valid_o = multdiv_valid;
      ALU_RES:         ex_valid_o = alu_valid;
      default:         ex_valid_o = alu_valid;
    endcase
  end  
